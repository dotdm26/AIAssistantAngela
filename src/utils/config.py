import os

from dotenv import load_dotenv

load_dotenv()

GOOGLE_API_KEY = os.getenv("TEST_KEY2")
LOCAL_EMBEDDING_MODEL = os.getenv("LOCAL_EMBEDDING_MODEL", "nomic-ai/nomic-embed-text-v1.5")

HYBRID_TOP_K = max(1, int(os.getenv("HYBRID_TOP_K", "5")))
HYBRID_MIN_PROMPT_CHARS = max(1, int(os.getenv("HYBRID_MIN_PROMPT_CHARS", "24")))
HYBRID_EXCLUDE_RECENT_COUNT = max(0, int(os.getenv("HYBRID_EXCLUDE_RECENT_COUNT", "4")))
HYBRID_MIN_SCORE = max(0.0, min(1.0, float(os.getenv("HYBRID_MIN_SCORE", "0.25"))))
HYBRID_MIN_SCORE_MEMORY = max(0.0, min(1.0, float(os.getenv("HYBRID_MIN_SCORE_MEMORY", "0.15"))))
COMMAND_MEMORY_MAX_PROMPT_LEN = max(1, int(os.getenv("COMMAND_MEMORY_MAX_PROMPT_LEN", "16")))
COMMAND_SEMANTIC_MIN_SCORE = max(0.0, min(1.0, float(os.getenv("COMMAND_SEMANTIC_MIN_SCORE", "0.82"))))
COMMAND_SEMANTIC_MIN_MARGIN = max(0.0, min(1.0, float(os.getenv("COMMAND_SEMANTIC_MIN_MARGIN", "0.08"))))
COMMAND_SEMANTIC_MAX_WORDS = max(1, int(os.getenv("COMMAND_SEMANTIC_MAX_WORDS", "8")))

MAX_TOOL_ROUNDS = max(1, int(os.getenv("MAX_TOOL_ROUNDS", "3")))
TOOL_FILTER_MIN_SCORE = max(0.0, min(1.0, float(os.getenv("TOOL_FILTER_MIN_SCORE", "0.28"))))

config_values = {
    "HYBRID_TOP_K": HYBRID_TOP_K,
    "HYBRID_MIN_PROMPT_CHARS": HYBRID_MIN_PROMPT_CHARS,
    "HYBRID_EXCLUDE_RECENT_COUNT": HYBRID_EXCLUDE_RECENT_COUNT,
    "HYBRID_MIN_SCORE": HYBRID_MIN_SCORE,
    "HYBRID_MIN_SCORE_MEMORY": HYBRID_MIN_SCORE_MEMORY,
    "COMMAND_MEMORY_MAX_PROMPT_LEN": COMMAND_MEMORY_MAX_PROMPT_LEN,
    "COMMAND_SEMANTIC_MIN_SCORE": COMMAND_SEMANTIC_MIN_SCORE,
    "COMMAND_SEMANTIC_MIN_MARGIN": COMMAND_SEMANTIC_MIN_MARGIN,
    "COMMAND_SEMANTIC_MAX_WORDS": COMMAND_SEMANTIC_MAX_WORDS,
    "MAX_TOOL_ROUNDS": MAX_TOOL_ROUNDS,
    "TOOL_FILTER_MIN_SCORE": TOOL_FILTER_MIN_SCORE,
}
